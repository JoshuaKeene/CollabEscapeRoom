...
{
    [Header("Manager")]
    public static J_UIManager TheUI;
    public J_FirstPersonController TheCharInputs;
    ...
    public bool AreAnyUIsActive()
    {
        ...
        if (J_DocumentManager.TheManager.DocPannel.activeInHierarchy) return true;
        if (J_InventoryManager.TheInventory.InventoryPannel.activeInHierarchy) return true;
    ...
    internal void TooltipMessage(string message, float displayTime)
    {
        if (!InProgress)
        {
            ...
            Tooltip.GetComponent<Animator>().Play(J_AnimationManager.Tooltip_FadeIn, -1, 0f);
            J_AudioManager.GlobalSFXManager.PlaySFX(J_AudioManager.GlobalSFXManager.TooltipSFX, null);
    ...
    public IEnumerator DisplayDuration(float Duration)
    {
        ...
        Tooltip.GetComponent<Animator>().Play(J_AnimationManager.Tooltip_FadeOut, -1, 0f);
