...
public class J_Interactor : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!PhysicalObject && !J_UIManager.TheUI.AreAnyUIsActive())
        {
            if (Physics.Raycast(ray, out hit, InteractionLength))
            {
                ...
                if (hit.transform)
                {
                    ...
                    if (hit.transform.CompareTag("Interactable"))
                    {
                        ...
                        if (TargetObject.GetComponent<J_InteractiveObject>().CanBeInteractedWith)
                        {
                            LastRaycastedObject = RaycastObject.Interactable;
                            Tooltip.text = "[E]" + TargetObject.GetComponent<J_InteractiveObject>().Tooltip;
                        }
        ...
        if (_selection == null)
        {
            if (Selection)
            {
                if (Fade)
                {
                    Fade = false;
                    Selection = false;
                    Tooltip.GetComponent<Animator>().Play(J_AnimationManager.InteractPrompt_FadeOut, -1, 0f);
                }
            }
        }
        else
        {
            if (!Selection)
            {
                Selection = true;
                Fade = true;
                Tooltip.GetComponent<Animator>().Play(J_AnimationManager.InteractPrompt_FadeIn, -1, 0f);
        ...
        if (Input.GetKeyDown(KeyCode.E))
        {
            if(LastRaycastedObject == RaycastObject.Interactable)
            {
                ...
                TargetObject.GetComponent<J_InteractiveObject>().ExecuteInteractiveAction();
            }
            else if(LastRaycastedObject == RaycastObject.Pickupable)
            {
                ...
                J_InventoryManager.TheInventory.AddItem(TargetObject.GetComponent<J_InventoryPickup>().AssociatedItem);
            ...
            else if (LastRaycastedObject == RaycastObject.Document)
            {
                ...
                Debug.Log("READED");
                J_DocumentManager.TheManager.OpenDocumentPannel(TargetObject.GetComponent<J_DocumentScript>());
            }
        }
