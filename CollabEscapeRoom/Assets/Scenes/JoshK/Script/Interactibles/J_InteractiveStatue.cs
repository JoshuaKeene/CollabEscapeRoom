...
public class J_InteractiveStatue : J_InteractiveObject //Green, Green, Red, White, Red, Green
{
    ...
    public override void ExecuteInteractiveAction()
    {
        ...
        //Determine if player has Idol on interact
        if (J_InventoryManager.TheInventory.HasItem("Golden Idol")) { HasIdol(true); }
        else { HasIdol(false); }

        //Set to false so player is not immediately kicked out
        LastPhase = false;

        OpenUI();

        //Set Dialogue based on puzzle state
        if (LookingForIdol) { Statue.GetComponent<J_DialogueScript>().StartingDialogueBranch = 41; Phase = 10; }

        UpdatePhases();

        //Set Dialogue based on puzzle state
        if (RiddleSolved || IdolReturned) { Statue.GetComponent<J_DialogueScript>().StartingDialogueBranch = 42; StatueUI.GetComponent<J_StatueUIManager>().BtnDisableOverride(true); }

        //Initiate Dialogue
        Statue.GetComponent<J_DialogueScript>().DialogueInit();
    ...
    public void Update()
    {
        //Exit interface at any eligable time
        if ((Input.GetKeyDown(KeyCode.Q) && StatueUI.activeInHierarchy && CanExit) || (!Statue.GetComponent<J_DialogueScript>().IsTalking && LastPhaseEnd))
        {
            LastPhaseEnd = false;
            CloseUI();
        }

        //Update phase when dialogue phase complete
        if (Statue.GetComponent<J_DialogueScript>().UpdateCheck == true && StatueUI.GetComponent<J_StatueUIManager>().Continue == true)
        {
            StatueUI.GetComponent<J_StatueUIManager>().Continue = false;
            ...
            Phase++;
            StatueUI.GetComponent<Animator>().Play(J_AnimationManager.Statue_DialogueChoice_FadeOut, -1, 0f);
            StartCoroutine("WaitForFadeAnim");
        }

        //Determine when player is eligable to exit
        if (Statue.GetComponent<J_DialogueScript>().IsTalking) { CanExit = false; }
        else { CanExit = true; }
    }

    public void ButtonPress(int ButtonID)
    {
        //Set Button ID based on phase and get associated dialogue branch to begin dialogue
        ...
        Statue.GetComponent<J_DialogueScript>().StartingDialogueBranch = ButtonID;
    ...
    public void UpdatePhases()
    {
        if (Phase == 1)
        {
            StatueUI.GetComponent<J_StatueUIManager>().SetDialogueOptions(DialogueCache[1], DialogueCache[2], DialogueCache[3], DialogueCache[4], 2, "Statue:");
            //Last Phase = true;
        }
        else if (Phase == 2)
        {
            StatueUI.GetComponent<J_StatueUIManager>().SetDialogueOptions(DialogueCache[5], DialogueCache[6], DialogueCache[7], DialogueCache[8], 2, "Statue:");
            LastPhase = false;
        }
        else if (Phase == 3)
        {
            StatueUI.GetComponent<J_StatueUIManager>().SetDialogueOptions(DialogueCache[9], DialogueCache[10], DialogueCache[11], DialogueCache[12], 3, "Statue:");
            LastPhase = false;
        }
        else if (Phase == 4)
        {
            StatueUI.GetComponent<J_StatueUIManager>().SetDialogueOptions(DialogueCache[13], DialogueCache[14], DialogueCache[15], DialogueCache[16], 1, "Statue:");
            LastPhase = false;
        }
        else if (Phase == 5)
        {
            StatueUI.GetComponent<J_StatueUIManager>().SetDialogueOptions(DialogueCache[17], DialogueCache[18], DialogueCache[19], DialogueCache[20], 2, "Statue:");
            LastPhase = false;
        }
        else if (Phase == 6)
        {
            StatueUI.GetComponent<J_StatueUIManager>().SetDialogueOptions(DialogueCache[21], DialogueCache[22], DialogueCache[23], DialogueCache[24], 1, "Statue:");
            LastPhase = false;
        }
        else if (Phase == 7)
        {
            StatueUI.GetComponent<J_StatueUIManager>().SetDialogueOptions(DialogueCache[25], DialogueCache[26], DialogueCache[27], DialogueCache[28], 5, "Statue:"); //All (Hidden) (END 2)
            LastPhase = false;
        }
        else if (Phase == 8)
        {
            StatueUI.GetComponent<J_StatueUIManager>().SetDialogueOptions(DialogueCache[29], DialogueCache[30], DialogueCache[31], DialogueCache[32], 2, "Statue:");
            LastPhase = false;
        }
        else if (Phase == 9)
        {
            if (J_InventoryManager.TheInventory.HasItem("Golden Idol")) { StatueUI.GetComponent<J_StatueUIManager>().SetDialogueOptions(DialogueCache[33], DialogueCache[34], DialogueCache[35], DialogueCache[36], 4, "Statue:"); } //(END 4)
            else { StatueUI.GetComponent<J_StatueUIManager>().SetDialogueOptions(DialogueCache[33], DialogueCache[34], DialogueCache[35], DialogueCache[36], 3, "Statue:"); } //(END 3)
            LastPhase = true;
            if (J_InventoryManager.TheInventory.HasItem("Golden Idol")) { hasIdol = true; }
        }
        else if (Phase == 10)
        {
            StatueUI.GetComponent<J_StatueUIManager>().SetDialogueOptions(DialogueCache[37], DialogueCache[38], DialogueCache[39], DialogueCache[40], 2, "Statue:");
            LastPhase = true;
            if (J_InventoryManager.TheInventory.HasItem("Golden Idol")) { hasIdol = true; }
        }
    }
    ...
    {
        //Update Cache based on puzzle state
    ...
    public void OpenUI()
    {
        ...
        Statue.GetComponent<J_DialogueScript>().ContinuousMode = false;

        StatueUI.GetComponent<Animator>().Play(J_AnimationManager.Statue_DialogueBox_FadeIn, -1, 0f);
    }

    public void CloseUI()
    {
        StatueUI.GetComponent<Animator>().Play(J_AnimationManager.Statue_DialogueBox_FadeOut, -1, 0f);
    ...
    public IEnumerator WaitForAnimClose()
    {
        ...
        Statue.GetComponent<J_DialogueScript>().ContinuousMode = true;
    ...
    public IEnumerator WaitForFadeAnim()
    {
        ...
        UpdatePhases();
        StatueUI.GetComponent<Animator>().Play(J_AnimationManager.Statue_DialogueChoice_FadeIn, -1, 0f);
    }
}
