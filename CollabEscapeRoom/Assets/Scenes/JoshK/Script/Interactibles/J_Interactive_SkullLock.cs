...
public class J_Interactive_SkullLock : J_InteractiveObject
{
    //Associated UI
    ...
    public override void ExecuteInteractiveAction()
    {
        ...
        InteractablesUIPannel.GetComponent<J_SkullLockManager>().OpenUI();
        if (firstInteraction) { J_UIManager.TheUI.TooltipMessage("Click the eyes to cycle colours.\n'Enter' to try a combination.", 3); firstInteraction = false; }
        ...
        if (GameObject.Find("Statue").gameObject.GetComponent<J_InteractiveStatue>().RiddleSolved || GameObject.Find("Statue").gameObject.GetComponent<J_InteractiveStatue>().IdolReturned)
        {
            J_UIManager.TheUI.TooltipMessage("Green, Green, Red, White, Red, Green.", 5f);
        }
    }
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Return))
        {
            ...
            if (Combination_1 != null)
            {
                ...
                else
                {
                    ...
                    J_UIManager.TheUI.TooltipMessage("Incorrect Combination.", 1f);
                }
            }
        }
        ...
        if (Input.GetKeyDown(KeyCode.Q) && ChestCam.activeInHierarchy)
        {
            ...
            Cursor.visible = false;
            Cursor.lockState = CursorLockMode.Locked;
            InteractablesUIPannel.GetComponent<J_SkullLockManager>().CloseUI();
    ...
    void OpenChest()
    {
        ...
        InteractablesUIPannel.GetComponent<J_SkullLockManager>().CloseUI();
        //Play open animation
        ChestOpenAnim.Play(J_AnimationManager.SkullLockChest_Open);
    ...
    public void BtnClick_01()
    {
        ...
        if (Type == ChestTypeEnum.Advanced)
        {
            gameObject.GetComponent<AudioSource>().PlayOneShot(J_AudioManager.GlobalSFXManager.SkullLockEyeClick);
