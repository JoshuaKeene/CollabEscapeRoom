...
public class J_StatueUIManager : MonoBehaviour
{
    ...
    public void Update()
    {
        if (Statue.GetComponent<J_DialogueScript>().IsTalking) { RaycastBlocker.SetActive(true); }
    ...
    {
        Statue.GetComponent<J_InteractiveStatue>().ButtonPress(1);
        if (ContinueID == 1 || AllContinue) { Continue = true; }
        if (AllContinue) { AllContinue = false; Statue.GetComponent<J_InteractiveStatue>().AllContinueBtnOne = true; }
    }

    public void OnBtnClick_OpTwo()
    {
        Statue.GetComponent<J_InteractiveStatue>().ButtonPress(2);
        if (ContinueID == 2 || AllContinue) { Continue = true; }
        if (AllContinue) { AllContinue = false; Statue.GetComponent<J_InteractiveStatue>().AllContinueBtnTwo = true; }
        if (Statue.GetComponent<J_InteractiveStatue>().hasIdol) 
        { 
            Statue.GetComponent<J_InteractiveStatue>().hasGivenIdol = true;
            Statue.transform.Find("Gold Idol").gameObject.SetActive(true);
            J_InventoryManager.TheInventory.RemoveItem("Golden Idol");
        }
    }

    public void OnBtnClick_OpThree()
    {
        Statue.GetComponent<J_InteractiveStatue>().ButtonPress(3);
        if (ContinueID == 3 || AllContinue) { Continue = true; }
        if (AllContinue) { AllContinue = false; Statue.GetComponent<J_InteractiveStatue>().AllContinueBtnThree = true; }
    }

    public void OnBtnClick_OpFour()
    {
        Statue.GetComponent<J_InteractiveStatue>().ButtonPress(4);
        if (ContinueID == 4 || AllContinue) { Continue = true; }
        if (AllContinue) { AllContinue = false; Statue.GetComponent<J_InteractiveStatue>().AllContinueBtnFour = true; }
        if (Statue.GetComponent<J_InteractiveStatue>().hasIdol)
        {
            Statue.GetComponent<J_InteractiveStatue>().hasGivenIdol = true;
            Statue.transform.Find("Gold Idol").gameObject.SetActive(true);
